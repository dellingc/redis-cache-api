...
const apiFetchRedis = (url, res, client, redisKey) => {
    console.log('clientConnected in apiFetchRedis = ' + clientConnected)
...
app.get('/loc', (req, res) => {
    console.log('clientConnected in app.get = ' + clientConnected)
        ...
        apiFetchRedis(url, res)
            ...
            return client.get(redisKey, (data) => {
                if (data) { // If that key exists in Redis store return that data to the client
                    ...
                    apiFetchRedis(url, res, client, redisKey)
                }
            })
    }
})
